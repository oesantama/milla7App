...
export default function ConductoresPage() {
  ...
  const [showFilters, setShowFilters] = useState(false); // P14
  const [filterEstado, setFilterEstado] = useState('all'); // P14: all, active, inactive
  ...
  const filteredAndSortedConductores = conductores.filter(c => {
      ...
      if (filterEstado === 'active') matchesEstado = c.activo === true;
      if (filterEstado === 'inactive') matchesEstado = c.activo === false;
      
      return matchesSearch && matchesEstado;
  }).sort((a, b) => {
    let aVal = a[sortColumn]; let bVal = b[sortColumn];
    if (sortColumn === 'activo') { aVal = aVal ? 1 : 0; bVal = bVal ? 1 : 0; } else if (sortColumn === 'fecha_creacion' || sortColumn === 'fecha_modificacion') { aVal = new Date(aVal).getTime(); bVal = new Date(bVal).getTime(); } else if (typeof aVal === 'string') { aVal = aVal.toLowerCase(); bVal = bVal.toLowerCase(); }
  ...
  const handleExport = () => {
    const dataToExport = filteredAndSortedConductores.map(c => ({ Cédula: c.cedula, Nombre: c.nombre, Celular: c.celular, Licencia: c.licencia, Activo: c.activo ? 'Sí' : 'No', 'Fecha de Creación': formatDate(c.fecha_creacion), 'Última Modificación': formatDate(c.fecha_modificacion) }));
  ...
  return (
    <div style={{padding:'30px',maxWidth:'1600px',margin:'0 auto',fontFamily:'Inter, sans-serif'}}>
      ...
      {/* Styles for Responsive behaviors */}
      <style jsx>{`
        ...
        <table style={{width:'100%',borderCollapse:'collapse'}}>
          <thead>
            <tr>
              ...
              <th style={{background:'linear-gradient(135deg, #1565c0 0%, #1976d2 100%)',color:'#fff',padding:'16px',textAlign:'left',fontWeight:'600',fontSize:'14px',textTransform:'uppercase',letterSpacing:'0.5px',cursor:'pointer',userSelect:'none',whiteSpace:'nowrap'}} onClick={()=>handleSort('activo')}>Activo {sortColumn==='activo'&&(sortDirection==='asc'?'▲':'▼')}</th>
          ...
          <tbody>
            {currentItems.map(c=>(
              <tr key={c.cedula} style={{borderBottom:'1px solid #f0f0f0'}}>
                ...
                <td style={{padding:'16px',fontSize:'14px',color:'#333'}}>{c.licencia}</td>
                <td style={{padding:'16px',fontSize:'14px',color:'#333'}}>
                  {c.eliminado ? (
                      ...
                      <span style={{padding:'6px 14px',borderRadius:'20px',fontSize:'13px',fontWeight:'600',display:'inline-block',background:c.activo?'#e8f5e9':'#ffebee',color:c.activo?'#2e7d32':'#c62828'}}>{c.activo?'Activo':'Inactivo'}</span>
                  )}
                </td>
        ...
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:'20px',padding:'10px',background:'#fff',borderRadius:'8px',boxShadow:'0 2px 8px rgba(0,0,0,0.05)'}}>
            ...
            <div style={{display:'flex',gap:'5px'}}>
                ...
                {Array.from({length: Math.min(5, Math.ceil(filteredAndSortedConductores.length / itemsPerPage))}, (_, i) => {
                    // Logic to show generic page numbers if many pages, but simple for now
                    let pageNum = i + 1; 
