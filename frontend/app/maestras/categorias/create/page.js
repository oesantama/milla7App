...
export default function CreateCategoriaPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('/api/core/categorias/', formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
      router.push('/maestras/categorias');
    } catch (err) {
    ...
    <div style={styles.container}>
      <div style={styles.header}>
        <h1 style={styles.title}>
          <i className="fa fa-tags" style={{ marginRight: '10px' }}></i>
          Crear Categoría
        </h1>
        <button onClick={() => router.push('/maestras/categorias')} style={styles.backButton}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={saving} style={{...styles.submitButton, opacity: saving ? 0.7 : 1}}>
            {saving ? (
              ...
              <span><i className="fa fa-save" style={{ marginRight: '8px' }}></i>Guardar Categoría</span>
            )}
          </button>
        </form>
      </div>
